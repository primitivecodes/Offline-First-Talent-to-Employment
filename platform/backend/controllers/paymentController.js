...
// ── DEMO bypass — phone 1111111111 grants instant access
const DEMO_PHONE = '1111111111';
  ...
  try {
    ...
    if (phone.replace(/\s/g, '') === DEMO_PHONE) {
      ...
      const payment = await Payment.create({
        userId:          user.id,
        type:            'learner_access',
        amount:          LEARNER_FEE_USD,
        currency:        'USD',
        amountUSD:       LEARNER_FEE_USD,
        phone,
        momoReferenceId: `DEMO-${Date.now()}`,
        status:          'successful',
        paidAt:          now,
      });
      await user.update({ hasPaidAccess: true });
      console.log('[DEMO] Learner access granted instantly for:', user.email);
      return res.status(200).json({
        status:    'successful',
        paymentId: payment.id,
        message:   'Demo access granted! You now have full access to all courses.',
        demo:      true,
      });
    }

    // ── REAL MTN MoMo payment ──────────────────────────
...
// ── Employer: initiate $20/month subscription ──────────
const initiateEmployerPayment = async (req, res) => {
  try {
    ...
    const existing = await Payment.count({
      where: { userId: user.id, type: 'employer_subscription', status: 'successful' },
    });
    const subscriptionMonth = existing + 1;

    // ── DEMO MODE: instant subscription ───────────────
    if (phone.replace(/\s/g, '') === DEMO_PHONE) {
      const payment = await Payment.create({
        userId:           user.id,
        type:             'employer_subscription',
        amount:           EMPLOYER_FEE_USD,
        currency:         'USD',
      ...
      console.log('[DEMO] Employer subscription granted instantly for:', user.email);
      return res.status(200).json({
        status:    'successful',
        paymentId: payment.id,
        message:   `Demo subscription activated until ${periodEnd.toLocaleDateString()}.`,
        demo:      true,
      });
    }
